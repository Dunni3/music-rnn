...
def get_preprocessed_files(processed_data_dir):
    ...
    for midi_dir in processed_data_dir.glob('*/'):
        ...
        roll = pd.read_csv(data_file, index_col=0).values
        preprocessed_data['roll_length'].append(roll.shape[0])
...
if __name__ == "__main__":
    ...
    print(f'dataset length: {len(dset_train)}')
    ...
    while iter_idx < n_iters:
        ...
        # record losses and # frames that are correct
        train_losses.append(loss.item())
        with torch.no_grad():
            notes = (pred > 0.5).type(torch.float)
            equal = torch.eq(notes, labels)
            train_frames_correct += torch.sum(torch.all(equal, axis=1))
            train_frames += features.shape[0]
        ...
        if iter_idx % out_interval == 0:
            ...
            # compute frac train frames correct
            frac_train_frames_correct = train_frames_correct / train_frames
            metrics['frac_train_frames_correct'].append(frac_train_frames_correct)
            train_frames_correct = 0
            train_frames = 0
