...
def get_roll_as_array(file_path):
...
def get_preprocessed_files(processed_data_dir):
    ...
    for midi_dir in processed_data_dir.glob('*/'):
        ...
        chord_roll = get_roll_as_array(data_file)
...
class PreprocessedChordRoll(Dataset):
    ...
    def get_rolls(self, file_idx, window_idx, idx):
        ...
        if file_idx in self.roll_cache:
            ...
            roll = get_roll_as_array(file_path)
...
if __name__ == "__main__":
    ...
    dset_train = PreprocessedChordRoll(df_meta=df_train, 
    ...
    dset_test = PreprocessedChordRoll(df_meta=df_test, 
                        seq_length=seq_length,
                        max_windows=20)
    ...
    model = ChordLSTM(hidden_size=hidden_size, vocab_size=60)
        ...
        if iter_idx % out_interval == 0:
            ...
            with torch.no_grad():
                for features, labels in test_dataloader:
                    ...
                    pred_chords = pred_softmax.argmax(axis=1)
